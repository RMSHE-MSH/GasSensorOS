...
`GasSensorOS` 是基于 `ESP32` 系列模块开发的嵌入式操作系统。项目包含多个模块，如数据结构容器（包括数据表和树形结构）、OLED 显示驱动、真随机数生成与加密算法、内核任务管理、I/O 操作、傅里叶变换工具，以及网络服务器等功能。项目使用 `C/C++` 语言编写，强调模块化、通用化设计和硬件抽象，便于开发高效的物联网应用。
...
| 编号 | 发现时间   | 位置                                                | 症状                                              | 原因                                                         | 修复情况 |
| ---- | ---------- | --------------------------------------------------- | ------------------------------------------------- | ------------------------------------------------------------ | -------- |
| 01   | 2024.08.08 | `io.cpp` - `void GPIOs::gpioToggleBit(uint8_t pin)` | 该函数无法将指定输出引脚的电平反转(原本是可以的). | 将 `PlatformIO Espressif 32` 更新到 `6.8.1` 版本后出现此问题. | 未修复   |

---
## 硬件适配情况

- **完全适配:** `ESP32-WROOM-32E`, `ESP32-WROOM-32D`
- **经测试部分适配:** `ESP-12F`, `ESP-01`
- **未测试部分适配:** `ESP8266EX`, `ESP32-S3-WROOM-1`
