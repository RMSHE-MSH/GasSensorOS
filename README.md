...
GasSensorOS 是基于 ESP32 系列模组开发的嵌入式操作系统。项目涵盖多个模块，包括数据结构容器（如数据表和树形结构）、OLED 显示屏驱动、真随机数生成和加密算法、内核任务管理、I/O 操作、傅里叶变换工具，以及网络服务器等功能。项目采用 C/C++ 语言编写，强调模块化设计和硬件抽象，便于开发高效的物联网应用。**项目目前仍处于早期开发阶段**，采用 AGPL-3.0 开源协议发布。
