...
---
## 🛜 WiFi Connector Commands

| 命令& 语法                                 | 说明                                                         | 参数说明                                                     | 示例                                                         |
| ------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| `wifi_connect [SSID] [PASSWORD] [timeout]` | 连接到指定的 Wi-Fi 网络。如果没有提供参数，则将自动尝试连接所有已保存的Wi-Fi凭据。 | `[SSID]`: 可选。Wi-Fi 网络的名称。<br />`[PASSWORD]`: 可选。Wi-Fi 网络的密码。<br /> `[timeout]`: 可选。连接超时时间（单位：毫秒），若不指定，将使用默认值 `10000`。 | `wifi_connect`: 自动连接到已保存的可用的Wi-Fi网络;<br/>`wifi_connect RMSHE 114514`: 连接到名称为 RMSHE , 密码为 114514 的 WIFI 网络;<br/>`wifi_connect RMSHE 114514 10000`: 连接到名称为 RMSHE , 密码为 114514 的 WIFI 网络, WIFI 连接的超时时间为 10s, 超出此时间则视为连接失败; |
| `ping [target]`                            | 测试与目标域名、主机或IP地址的网络连通性。<br />当不提供 `target` 参数时，通过 ping 默认公共 DNS ( `8.8.8.8`) 来快速检测设备的互联网访问能力,(包括是否连接 WIFI, 以及是否可连接到互联网)。 | `[target]`: 可选。目标URL、主机名或 IP 地址。                | `ping`: 检测设备是否能访问互联网;<br />`ping https://www.google.com/`: 检测与特定域名的连通性;<br />`ping 8.8.4.4`: 检测与特定 IP 地址的连通性; |
