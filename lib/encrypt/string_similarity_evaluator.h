...
class StringSimilarityEvaluator {
   private:
    ...
    // 权重定义，可以根据需求进行调整
    const float weightCosine = 0.40;       // 余弦相似度权重
   ...
   public:
     ...
     * @return 返回两个字符串的相似度
     */
    float nGramSimilarity(uint8_t n);
   ...
   private:
     ...
     * @return 返回两个集合的相似度
     */
    float calculateNGram(const std::unordered_set<std::string> &set1, const std::unordered_set<std::string> &set2);

    // 文本预处理：将字符串转换为小写，并计算单词频率
    std::unordered_map<std::string, int> getWordFrequency(const std::string &input);

    // 计算向量的模
    float calculateMagnitude(const std::unordered_map<std::string, int> &freq);
};
