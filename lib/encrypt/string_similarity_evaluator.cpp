...
float StringSimilarityEvaluator::evaluateStringSimilarity() {
    // 计算原始相似度和距离
    float cosine = cosineSimilarity();                                  // 余弦相似度
    float nGram = nGramSimilarity(selectNGramLengthBasedOnStrength());  // 计算N-gram相似度(基于密码强度动态选择N-gram长度)
    int levenshtein = levenshteinDistance();                            // 莱文斯坦距离

    // 将莱文斯坦距离转换为相似度得分
    // 这里距离越小，相似度越高
    float levenshteinSimilarity = 1.0 / (1.0 + levenshtein);

    // 将余弦相似度转换为[0, 1]范围的得分，以便与其他得分统一处理
    float cosineScore = (cosine + 1) / 2;

    // 计算综合评估得分
    // 使用加权平均方法，将不同相似度和距离度量综合在一起
    float totalScore = weightCosine * cosineScore + weightNGram * nGram + weightLevenshtein * levenshteinSimilarity;
 ...
 * @return 返回两个字符串的相似度
 */
float StringSimilarityEvaluator::nGramSimilarity(uint8_t n) {
    // 生成N-gram集合
...
float StringSimilarityEvaluator::cosineSimilarity() {
    ...
    // 计算模并应用余弦相似度公式
    float magnitude1 = calculateMagnitude(freq1);
    float magnitude2 = calculateMagnitude(freq2);
...
uint8_t StringSimilarityEvaluator::selectNGramLengthBasedOnStrength() {
    ...
    // 计算两个密码的强度评分并求平均值
    float avgScore = (Strength.evaluateString(str1) + Strength.evaluateString(str2)) / 2.0;
 ...
 * @return 返回两个集合的相似度
 */
float StringSimilarityEvaluator::calculateNGram(const std::unordered_set<std::string> &set1, const std::unordered_set<std::string> &set2) {
...
// 计算向量的模
float StringSimilarityEvaluator::calculateMagnitude(const std::unordered_map<std::string, int> &freq) {
    float magnitude = 0.0;
